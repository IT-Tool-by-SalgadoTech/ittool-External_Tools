...
#  Triggered AFTER the user types ITTOOL — not before.
#  Closes the port with HUPCL active to drop DTR → ESP32 resets
#  into Script_Saver mode. While it boots, the packet is built.
# ============================================================
echo ""
echo -e "\e[33mIT-Tool reset — go to: ReadyUSB > Script_Saver\e[0m"
...
try:
    ...
    attrs[2] |= termios.HUPCL   # enable: closing drops DTR → ESP32 resets
    termios.tcsetattr(fd, termios.TCSANOW, attrs)
    time.sleep(0.1)
    os.close(fd)                # DTR drops here → ESP32 resets
    time.sleep(0.5)
    print("Reset sent.")
except Exception as e:
    print(f"Reset warning: {e}", file=sys.stderr)
PYSCRIPT
echo -e "\e[32mWaiting for IT-Tool to boot into Script_Saver...\e[0m"
echo ""
