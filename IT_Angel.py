...
def capture_tshark_background(cycle_num, duration_sec, result_holder, done_event):
    """
    Capture traffic to a .pcap file, then convert to text at the end.
    Most reliable approach on Windows — no stdout buffering issues.
    """
    if not g_tshark_path:
        done_event.set()
        return
    ...
    pcap_file = os.path.join(tmp_dir, f"_it_angel_{cycle_num}.pcap")

    try:
        # Step 1: Capture to .pcap file for the full duration
        cap_cmd = [g_tshark_path,
                   "-i", iface,
                   "-a", f"duration:{duration_sec}",
                   "-w", pcap_file]
        proc = subprocess.Popen(cap_cmd, stderr=subprocess.DEVNULL)
        try:
            proc.wait(timeout=duration_sec + 30)
        except subprocess.TimeoutExpired:
            try: proc.kill(); proc.wait(timeout=5)
            except: pass

        # Step 2: Convert .pcap to text fields
        if not os.path.exists(pcap_file):
            return

        read_cmd = [g_tshark_path,
                    "-r", pcap_file,
                    "-T", "fields",
                    "-e", "frame.time_epoch",
                    "-e", "ip.src",
                    ...
                    "-E", "quote=n",
                    "-E", "occurrence=f"]
        r = subprocess.run(read_cmd, capture_output=True, text=True, timeout=60)
        ...
        for line in r.stdout.splitlines():
            ...
            if src or dst:
                row = {"Timestamp": ts, "Cycle": cycle_num,
...
def save_traffic_to_excel(traffic_rows):
    """Append traffic rows to the Network Traffic sheet."""
    if not traffic_rows:
        return
    try:
        wb     = openpyxl.load_workbook(g_excel_path)
...
def show_live_traffic_and_countdown(wait_seconds, cycle_num, duration_end=None):
    ...
    # Poll with 1s timeout so Ctrl+C is still caught quickly
    while not _display_done.wait(timeout=1.0):
        if g_stop_event.is_set():
            break

    # ── Wait for tshark to finish and save ────────────────────────────────────
    if g_tshark_path:
        wait_extra = 15 if os.name == 'nt' else 10
        tshark_done.wait(timeout=wait_extra)
        if tshark_rows:
            save_traffic_to_excel(tshark_rows)
