...
def check_open_shares(cycle):
    ...
    if g_os_target == "windows":
        ...
        for line in out.splitlines():
            ...
            # Skip smbstatus header/separator lines
            if line.startswith("-") or line.startswith("Service") or line.startswith("Samba") \
               or line.startswith("PID") or line.startswith("="):
                continue
...
def check_privileged_processes(cycle):
    ...
    EXPECTED_ROOT_LIN = {
        ...
        # Bluetooth
        "krfcommd",
        # XFCE desktop session processes (running as root on Kali)
        "thunar","dbus-launch","xfconfd","tumblerd","xfwm4","xfdesktop","xfce4-panel",
        "xfce4-session","xfsettingsd","xfce4-notifyd","xfce4-power-man","xfce4-screensa",
        "xdg-desktop-por","xdg-permission-","xdg-document-po",
        # GVFS virtual filesystem daemons (normal root processes in XFCE/GNOME)
        "gvfsd","gvfsd-fuse","gvfsd-trash","gvfsd-network","gvfsd-computer",
        "gvfs-udisks2-vo","gvfs-gphoto2-vo","gvfs-afc-volume","gvfs-goa-volume","gvfs-mtp-volume",
        # Other common desktop daemons
        "at-spi-bus-laun","at-spi2-registr","ibus-daemon","ibus-x11","ibus-portal",
        "gsd-xsettings","gsd-media-keys","gsd-power","gsd-color","gsd-print-notif",
        "gnome-keyring-d","gnome-session-b","evolution-sourc","evolution-calen",
        "pipewire","pipewire-pulse","wireplumber","pulseaudio","pactl",
        "Xorg","xrandr","xrdb","xsetroot","xset","dbus-run-session",
    }
