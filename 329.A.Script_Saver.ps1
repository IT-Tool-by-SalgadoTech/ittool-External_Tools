...
elseif ($system -eq "Linux") {
    ...
    while ([string]::IsNullOrWhiteSpace($targetFolder)) {
        $fc = (Read-Host "Folder number").Trim()
        switch ($fc) {
            "1" { $targetFolder = "B.OS_System/B.Linux/A.Admin_And_Security" }
            "2" { $targetFolder = "B.OS_System/B.Linux/B.Networks" }
            "3" { $targetFolder = "B.OS_System/B.Linux/C.Folders_and_Files" }
            "4" { $targetFolder = "B.OS_System/B.Linux/D.Storage" }
            "5" { $targetFolder = "B.OS_System/B.Linux/E.Monitoring" }
            "6" { $targetFolder = "B.OS_System/B.Linux/F.External_links_tools" }
            "7" { $targetFolder = "B.OS_System/B.Linux/G.Nmap" }
            "8" { $targetFolder = "B.OS_System/B.Linux/H.Kali_Linux" }
            "0" { $targetFolder = "Favorites" }
            default { Write-Host "  Invalid option." -ForegroundColor Yellow }
        }
    }
